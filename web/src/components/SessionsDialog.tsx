...
import { deleteSession, getSessions, Session } from '../api';
  ...
  const onDeleteClicked = useCallback(
    (sessionId: string) => {
      deleteSession(sessionId).then(() => {
        if (
          pageOffset * PAGE_SIZE + sessions.length >=
          totalSessions
        ) {
          requestSessions(pageOffset - 1);
        } else {
            ...
            <Table.Body>
              {sessions.map((session) => (
                ...
                >
                  ...
                  <Table.TextCell>{session.updatedAt}</Table.TextCell>
